...
var Actor, CanvasRendererComponent, CyanGhost, Direction, GameMode, Ghost, GhostMode, KeyboardInputComponent, Kovas, Map, OrangeGhost, PinkGhost, Player, RedGhost, Scorer, Tile,
...
CanvasRendererComponent = (function() {
  ...
  CanvasRendererComponent.prototype.drawLives = function(lives) {
    ...
    this.ctx.translate(0, -20);
    this.ctx.fillStyle = '#fff';
    this.ctx.fillText(Array(lives + 1).join("♥"), 500, 0);
    return this.ctx.restore();
  };
...
Ghost = (function(_super) {
  ...
  Ghost.prototype.MODE_DELAY_CHASE = 60 * 30;

  Ghost.prototype.MODE_DELAY_SCATTER = 60 * 5;

  function Ghost(map, player) {
    this.map = map;
    this.player = player;
    this.mode_delay = this.MODE_DELAY_SCATTER;
  ...
  Ghost.prototype.update = function() {
    ...
    if (this.nextMoveIn <= 0) {
      ...
      if (newPosition) {
        ...
        if (this.mode === GhostMode.scatter) {
          this.nextMoveIn = this.moveDelay() / 2;
        } else {
          this.nextMoveIn = this.moveDelay();
        }
        this.movedTile();
    ...
    this.mode_delay -= 1;
    if (this.mode_delay <= 0) {
      if (this.mode === GhostMode.scatter) {
        this.mode = GhostMode.chase;
        this.mode_delay = this.MODE_DELAY_CHASE;
      } else {
        this.mode = GhostMode.scatter;
        this.mode_delay = this.MODE_DELAY_SCATTER;
      }
      return this.updateTargetting(true);
    }
  };

  Ghost.prototype.movedTile = function() {
    return this.updateTargetting();
  };

  Ghost.prototype.updateTargetting = function(canReverse) {
    var distances, target, x, y, _ref, _ref1, _ref2, _ref3;
    if (canReverse == null) {
      canReverse = false;
    }
    if (canReverse) {
      this.canMoveUp = this.canMove(Direction.up);
      this.canMoveLeft = this.canMove(Direction.left);
      this.canMoveDown = this.canMove(Direction.down);
      this.canMoveRight = this.canMove(Direction.right);
    } else {
  ...
  Ghost.prototype.respawn = function() {
    Ghost.__super__.respawn.call(this);
    this.mode_delay = this.MODE_DELAY_SCATTER;
    this.mode = GhostMode.scatter;
    return this.updateTargetting();
  };
...
RedGhost = (function(_super) {
  ...
  RedGhost.prototype.customTarget = function() {
    return this.player;
  };
...
PinkGhost = (function(_super) {
  ...
  PinkGhost.prototype.customTarget = function() {
    var x, y, _ref;
...
CyanGhost = (function(_super) {
  ...
  CyanGhost.prototype.customTarget = function() {
    var x, y, _ref;
...
OrangeGhost = (function(_super) {
  ...
  OrangeGhost.prototype.customTarget = function() {
    if (this.targeting_mode === GhostMode.chase) {
      if (this.calculateDistance(this.x, this.y, this.player.x, this.player.y) < 8) {
        this.targeting_mode = GhostMode.scatter;
      }
      return this.player;
    } else {
      if (this.calculateDistance(this.x, this.y, this.player.x, this.player.y) >= 8) {
        this.targeting_mode = GhostMode.chase;
      }
      return this.chaseTarget();
    }
  };
