...
var Actor, CanvasRendererComponent, Direction, GameMode, Ghost, KeyboardInputComponent, Kovas, Map, Player, Scorer, Tile,
...
CanvasRendererComponent = (function() {
  ...
  CanvasRendererComponent.prototype.drawGhost = function(x, y) {
    x *= 25;
    y *= 25;
    this.ctx.save();
...
Ghost = (function(_super) {
  ...
  Ghost.prototype.movedTile = function() {
    var distances, x, y, _ref, _ref1, _ref2, _ref3;
    ...
    if (this.canMoveUp) {
      ...
      this.upDistance = this.calculateDistance(x, y, this.player.x, this.player.y);
    } else {
      this.upDistance = Infinity;
    }
    if (this.canMoveLeft) {
      _ref1 = this.newPosition(Direction.left), x = _ref1[0], y = _ref1[1];
      this.leftDistance = this.calculateDistance(x, y, this.player.x, this.player.y);
    } else {
      this.leftDistance = Infinity;
    }
    if (this.canMoveDown) {
      _ref2 = this.newPosition(Direction.down), x = _ref2[0], y = _ref2[1];
      this.downDistance = this.calculateDistance(x, y, this.player.x, this.player.y);
    } else {
      this.downDistance = Infinity;
    }
    if (this.canMoveRight) {
      _ref3 = this.newPosition(Direction.right), x = _ref3[0], y = _ref3[1];
      this.rightDistance = this.calculateDistance(x, y, this.player.x, this.player.y);
    } else {
...
Kovas = (function() {
  ...
  Kovas.prototype.gameStart = function() {
    ...
    this.ghosts = [new Ghost(this.map, this.player)];
  ...
  Kovas.prototype.update = function() {
    ...
    for (_l = 0, _len1 = _ref3.length; _l < _len1; _l++) {
      ghost = _ref3[_l];
      this.options.renderer.drawGhost(ghost.x, ghost.y);
    }
    if (this.mode !== GameMode.play) {
